...
描画順序（下→上）:
┌─────────────────────────────────────────────┐
│ 4. UI層（共通）  — Dialog, Menu, Dice       │  ← ビュー非依存
│ 3. HUD層（共通） — ターン情報, 所持金, Cmd  │  ← ビュー非依存
├─────────────────────────────────────────────┤
│ 2. プレイヤー層  — ビュー依存（切り替え対象）│  ← ViewManager管理
│ 1. ボード層      — ビュー依存（切り替え対象）│  ← ViewManager管理
└─────────────────────────────────────────────┘
```

**方針:** HUD（`src/ui/hud.py`）とUI（dialog, menu）はビューに依存しない共通レイヤーとする。
ViewManagerが管理するのはボード描画（層1）とプレイヤー描画（層2）のみ。
HUD/UIはシーン側で直接描画し、ビュー切り替えの影響を受けない。

これにより：
- HUDコードの重複を防止
- ビュー追加時にHUD/UIを再実装する必要なし
- 各ビューはゲーム世界の描画だけに専念
