...
class BattleScene(Scene):
    def __init__(self):
        ...
        self.selected = 0
        self.commands = ["Attack", "Defend", "Card", "Flee"]
        self.message = ""
    ...
    def _execute_command(self, cmd_index):
        ...
        if cmd_index == 0:  # Attack
            damage = self.battle.execute_attack(actor_stats, target_stats)
            self.message = f"{current.name}: {damage} damage!"
        elif cmd_index == 1:  # Defend
            self.battle.execute_defend(actor_stats)
            self.message = f"{current.name} is defending!"
            ...
            if battle_cards:
                ...
                self.message = f"Used {card.name}!"
            else:
                # No cards, just attack
                damage = self.battle.execute_attack(actor_stats, target_stats)
                self.message = f"No cards! {damage} damage!"
        elif cmd_index == 3:  # Flee
            if self.battle.try_flee(actor_stats):
                ...
                self.message = f"{current.name} fled! (-{penalty}$)"
                self.phase = BattlePhase.END
            else:
                self.message = f"{current.name} failed to flee!"
    ...
    def _handle_battle_end(self):
        if self.battle.fled:
            self.message = "Battle ended (fled)"
            self.message_timer = 30
            return
        ...
        if winner and loser and self.battle_tile and self.battle_tile.company:
            ...
            self.message = f"{winner.name} wins! Got {company.name}!"
        else:
            self.message = "Battle Over!"
